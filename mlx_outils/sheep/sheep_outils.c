...
/*   Updated: 2024/02/24 17:33:12 by ebaillot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_and_load_sheep(t_resources *res)
{
	...
	while (i < 4)
	{
		...
		free(temp);
		free(number_sheep);
		res->sheep->sprites[i] = mlx_xpm_file_to_image(res->data.mlx_ptr, str,
...
void	update_sheep_animation(t_sheep *sheep, int interval, int sprite_count)
{
	...
	if (sheep->sprites)
	{
		if (!sheep->is_collected)
		{
			mlx_put_image_to_window(data->mlx_ptr, data->win_ptr,
				sheep->sprites[sheep->current_sprite], sheep->x * 64, sheep->y
				* 64);
		}
	}
}
